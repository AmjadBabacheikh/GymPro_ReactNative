...
export const setInit = (result) => async (dispatch) => {
  dispatch({ type: SET_INIT, payload: JSON.parse(result) });
};
...
export const login = (email, password) => async (dispatch) => {
  try {
    ...
    dispatch({ type: USER_LOGIN_SUCCESS, payload: data.jwt });
    await storeToken('userInfo', JSON.stringify(data.jwt));
  } catch (error) {
    dispatch({
...
export const register =
  ...
  async (dispatch) => {
    try {
      ...
      dispatch({ type: USER_REGISTER_SUCCESS, payload: data.jwt });
      dispatch({ type: USER_LOGIN_SUCCESS, payload: data.jwt });
      await storeToken('userInfo', JSON.stringify(data.jwt));
    } catch (error) {
      dispatch({
...
export const getMyProfile = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`${baseUrl}/api/user/profil`, config);
...
export const getCart = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`${baseUrl}/api/client/cart`, config);
